...
import { buildExecutionContext, assertValidExecutionArguments, execute, } from 'graphql/execution/execute';
...
export const subscribe = async ({ server, event, message }) => {
    try {
...
const validateMessage = (server) => (message) => {
    ...
    try {
        assertValidExecutionArguments(server.schema, parse(message.payload.query), message.payload.variables);
    }
    catch (err) {
        return [err];
    }
